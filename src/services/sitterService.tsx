...
const API_URL = 'http://localhost:8088/api/sitter-profiles';
const USERS_API_URL = 'http://localhost:8088/api/users';
 ...
 */
export const createSitterProfile = async (profileData: CreateSitterProfileRequest): Promise<SitterProfileDTO> => {
    try {
        const response = await axios.post(API_URL, profileData, getAuthHeaders());
 ...
 */
export const getSitterProfile = async (userId: number): Promise<SitterProfileDTO> => {
    try {
 ...
 */
export const updateSitterProfile = async (userId: number, profileData: CreateSitterProfileRequest): Promise<SitterProfileDTO> => {
    try {
        const response = await axios.put(`${API_URL}/${userId}`, profileData, getAuthHeaders());
        return response.data;
    } catch (error) {
 ...
 */
export const registerSitter = async (sitterData: SitterRegisterRequest): Promise<AuthResponse> => {
    try {
        const response = await axios.post(`${USERS_API_URL}/register-sitter`, sitterData);
        return response.data;
    } catch (error) {
...
export const getAllSitters = async (): Promise<SitterProfileSummary[]> => {
    try {
        const response = await axios.get(`${USERS_API_URL}/role/SITTER`, getAuthHeaders());
...
export const searchSitters = async (filters: {
    ...
    try {
        ...
        return allSitters.filter(sitter => {
            ...
            if (filters.minRating && (sitter.averageRating || 0) < filters.minRating) return false;
            
            // Filtrar por tarifa máxima
            if (filters.maxHourlyRate && (sitter.hourlyRate || 0) > filters.maxHourlyRate) return false;
            
            // Filtrar por ubicación (búsqueda simple)
            if (filters.location && sitter.location && 
                !sitter.location.toLowerCase().includes(filters.location.toLowerCase())) return false;
...
const mapUserToSitter = (user: any): SitterProfileSummary => {
    return {
        ...
        profileImageUrl: null, // Se completará con el perfil
 ...
 */
const mapProfileToExtendedSitter = (profile: SitterProfileDTO): ExtendedSitter => {
    return {
        id: profile.id || 0,
        ...
        nextAvailable: 'Hoy a las ' + (12 + Math.floor(Math.random() * 8)) + ':00',
...
export const getSitterStats = async () => {
    try {
        ...
        const availableCount = allProfiles.filter(p => p.availableForBookings).length;
        ...
        return {
            ...
            averageHourlyRate: allProfiles.reduce((sum, p) => sum + (p.hourlyRate || 0), 0) / allProfiles.length || 0
        };
    } catch (error) {
...
export async function addMyService(serviceData: any) { // 'any' debería ser un tipo más específico
