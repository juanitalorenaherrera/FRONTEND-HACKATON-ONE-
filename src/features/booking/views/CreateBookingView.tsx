...
export const CreateBookingView = () => {
  const { form, loading, creationData, isSubmitting, actions } = useCreateBooking();
  ...
  return (
    <motion.div className="max-w-4xl mx-auto p-4 md:p-8 space-y-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      <header className="text-center">
        <h1 className="text-3xl font-bold text-pet-orange">Crear una Nueva Reserva</h1>
        <p className="text-neutral-500 mt-2">Sigue los pasos para encontrar el cuidado perfecto.</p>
      </header>
      <form onSubmit={(e: BaseSyntheticEvent) => e.preventDefault()}>
        <AnimatePresence mode="wait">
          ...
          {step === 2 && <CreateBookingStep2 form={form} services={sitterServices} isLoading={isLoadingServices} />}
          {step === 3 && <BookingSummaryCard formData={form.watch()} creationData={creationData} services={sitterServices} />}
        </AnimatePresence>
        <div className="flex justify-between items-center pt-8 mt-8 border-t border-neutral-200">
          <div>
            {step > 1 && <Button type="button" variant="outline" onClick={handleBack}>Anterior</Button>}
          </div>
          <div>
            {step < 3 ? (
              <Button type="button" onClick={handleNext}>Siguiente</Button>
            ) : (
              <Button type="button" onClick={actions.onSubmit} disabled={isSubmitting} variant="primary">
