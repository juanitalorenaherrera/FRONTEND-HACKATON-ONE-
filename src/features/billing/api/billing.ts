...
export const downloadInvoice = async (invoiceId: number): Promise<Blob> => {
	...
	const response = await axios.get(
		`${API_URL_INVOICES}/${invoiceId}/download`,
		{
			responseType: 'blob',
		}
	);
...
export const getPaymentMethodsByAccount = async (
	...
	const response = await axios.get(
		`${API_URL_PAYMENT_METHODS}/account/${accountId}`
	);
	return response.data;
};
...
export const addPaymentMethod = async (
	...
	const response = await axios.post(
		`${API_URL_PAYMENT_METHODS}/${accountId}`,
		methodData
	);
	return response.data;
};
...
export const updatePaymentMethod = async (
	...
	const response = await axios.put(
		`${API_URL_PAYMENT_METHODS}/${methodId}`,
		updateData
	);
	return response.data;
};
...
export const setDefaultPaymentMethod = async (
	...
	// Esta ruta es correcta.
	const response = await axios.put(
		`${API_URL_PAYMENT_METHODS}/${methodId}/default`
	);
	return response.data;
};
