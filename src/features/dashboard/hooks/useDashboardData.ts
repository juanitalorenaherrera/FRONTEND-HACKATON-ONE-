...
import type { Appointment, DashboardData, Pet, Sitter, Stats, UserProfile } from '../../../types/dashboardData';
...
export const useDashboardData = () => {
  ...
  const refetch = useCallback(() => fetchDashboardData(false), [fetchDashboardData]);
   ...
   */
  const userProfile = useMemo(() => dashboardData?.userProfile ?? defaultUserProfile, [dashboardData]);
  const userPets = useMemo(() => dashboardData?.userPets ?? [], [dashboardData]);
  const recentSitters = useMemo(() => dashboardData?.recentSitters ?? [], [dashboardData]);
  const stats = useMemo(() => dashboardData?.stats ?? defaultStats, [dashboardData]);
  const nextAppointment = useMemo(() => dashboardData?.nextAppointment ?? null, [dashboardData]);
