...
export function AdvancedFilters({ onClose }: AdvancedFiltersProps) {
  ...
  const [localFilters, setLocalFilters] = useState<SitterFilters>(state.filters);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={onClose} className="p-2 text-gray-500 hover:bg-gray-100 rounded-full">✕</button>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              <input
                ...
                value={localFilters.maxDistance || SITTER_CONFIG.FILTERS.DEFAULT_RADIUS}
                onChange={(e) => handleLocalFilterChange('maxDistance', parseInt(e.target.value))}
                className="flex-1"
              />
              <span className="text-sm font-medium w-16 text-gray-600">
                {localFilters.maxDistance || SITTER_CONFIG.FILTERS.DEFAULT_RADIUS} km
              </span>
            </div>
          </div>
          ...
          <div>
            <label className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-2">
              <DollarSign className="w-4 h-4" /> Tarifa máxima por hora
            </label>
            <input
              type="number"
              placeholder="Ej: 15000"
              value={localFilters.maxHourlyRate || ''}
              onChange={(e) => handleLocalFilterChange('maxHourlyRate', e.target.value ? parseInt(e.target.value) : undefined)}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {SITTER_CONFIG.SPECIALTIES.map((specialty) => (
                <button
                  key={specialty}
                  onClick={() => handleLocalFilterChange('specialty', localFilters.specialty === specialty ? undefined : specialty)}
        ...
        <div className="flex gap-3 mt-8 pt-6 border-t">
          <button onClick={handleReset} className="px-6 py-3 border border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 font-medium">
            Limpiar Todo
          </button>
          <button onClick={handleApply} className="flex-1 px-6 py-3 bg-orange-500 text-white rounded-xl hover:bg-orange-600 font-medium">
            Aplicar Filtros
          </button>
        </div>
      </div>
