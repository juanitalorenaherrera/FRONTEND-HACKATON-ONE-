...
export const ProfileForm: React.FC<ProfileFormProps> = ({ profile, onSave }) => {
  ...
  return (
    <motion.form
      ...
      className="bg-white/80 backdrop-blur-xl border border-white/30 p-8 rounded-2xl shadow-lg space-y-6"
    >
        <h2 className="text-2xl font-bold text-neutral-800 mb-4">Tu Información Personal</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label htmlFor="firstName" className="block text-sm font-medium text-neutral-600">Nombre</label>
                <Input id="firstName" {...register('firstName')} disabled={isSubmitting} />
                {errors.firstName && <p className="text-red-500 text-xs mt-1">{errors.firstName.message}</p>}
            </div>
            <div>
                <label htmlFor="lastName" className="block text-sm font-medium text-neutral-600">Apellido</label>
                <Input id="lastName" {...register('lastName')} disabled={isSubmitting} />
                {errors.lastName && <p className="text-red-500 text-xs mt-1">{errors.lastName.message}</p>}
            </div>
        </div>
        <div>
            <label htmlFor="phoneNumber" className="block text-sm font-medium text-neutral-600">Teléfono</label>
            <Input id="phoneNumber" {...register('phoneNumber')} disabled={isSubmitting} placeholder="Añade tu teléfono" />
        </div>
        <div>
            <label htmlFor="address" className="block text-sm font-medium text-neutral-600">Dirección</label>
