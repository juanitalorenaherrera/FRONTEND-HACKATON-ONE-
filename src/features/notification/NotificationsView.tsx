...
export const NotificationsView = () => {
  return (
    ...
    >
      <motion.header
        className="bg-gradient-secondary rounded-3xl p-8 mb-8 text-white shadow-xl"
        initial={{ y: -50, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ delay: 0.1, duration: 0.5 }}
      >
        <h1 className="text-3xl font-bold flex items-center gap-3"><Bell />Centro de Notificaciones</h1>
        <p className="text-white/80 mt-2">Todas tus alertas y mensajes importantes aquí.</p>
      </motion.header>
      
      <div className="bg-white rounded-2xl shadow-md border border-neutral-200 p-4 md:p-6 space-y-4">
          ...
          {notifications.map(n => {
            ...
            return (
              <motion.li
                ...
                className="flex items-start gap-4 p-4 rounded-xl transition-colors hover:bg-neutral-100"
              >
                <div className={cn("w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center text-white", IconConfig.color)}>
